...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection{The string hypothesis: Reduced overlap formulas}
...
configurations with no zero-momentum strings. Our results are not valid for 
zero-rapidity strings. 
...
These would require the knowledge of the precise form of the string 
...
In the thermodynamic limit the expression for the overlap~\eref{neel-ov} becomes 
%
\begin{equation}
\label{neel-ov-th}
\fl\lim_{L\to\infty}\left[\frac{\langle N|\{\pm\lambda_j\}_{j=1}^m,n_\infty
\rangle}{|||\{\lambda_j\}_{j=1}^m,n_\infty\rangle||}\right]=\frac{L}{2}\Big(
n_\infty\log(2)+\sum_{n=1}^\infty\int_0^\infty d\lambda\rho_n(\lambda)[g_n(\lambda)
+2n\log(4)]\Big), 
\end{equation}
%
with 
%
\begin{eqnarray}
g_n(\lambda)=\sum_{l=1}^{n-1}\Big[f_{n-1-2l}(\lambda)-f_{n-2l}(\lambda)\Big],\\
f_n(\lambda)=\log\Big(\lambda^2+\frac{n^2}{4}\Big),\\
n_\infty=1-2\sum_{m=1}^\infty m\int_{-\infty}^\infty d\lambda\rho_m(\lambda). 
\end{eqnarray}
%
Note that~\eref{neel-ov-th} is extensive, subextensive contributions 
are neglected. In particular, Eq.~\eref{neel-ov-th} that the ratio of 
determinants $\det_m(G^+)/\det_m(G^-)$ does not contribute. 
