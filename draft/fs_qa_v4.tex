...
Note that for the diagonal ensemble~\eref{d-ensemble}  one has trivially that $\langle
...
We focus only on the situation with the N\'eel state (cf.~\eref{psi0}) as 
initial state. The key idea is to sample the eigenstates with no zero-momentum 
strings with the diagonal ensemble probability measure $\rho^{DE}$ (cf.~\eref{de1}). 
...
\end{equation}
%
Putting everything together (\eref{wrong} and~\eref{wrong1}) one would get a 
diverging MG overlap, which is clearly impossible. 
