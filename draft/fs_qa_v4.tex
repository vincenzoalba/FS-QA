...
contemporary physics, both theoretically~\cite{polkovnikov-2011} and 
experimentally~\cite{bloch-2008,greiner-2002,kinoshita-2006,
hofferberth-2007,trotzky-2012,gring-2012,cheneau-2012,
schneider-2012,kunhert-2013,langen-2013,meinert-2013,fukuhara-2013,
ronzheimer-2013,braun-2014,langen-2015}. The most popular out-of-equilibrium 
protocol is that of the {\it quantum quench} in which a system is initially 
prepared in an eigenstate $|\Psi_0\rangle$ of a many-body hamiltonian ${\mathcal H}_0$. 
Then a global parameter is suddenly changed, and the system is let to evolve 
unitarily under a new hamiltonian ${\mathcal H}$. There is now compelling 
evidence that at long times after the quench an equilibrium steady state 
arises (see for instance Ref.~\cite{kinoshita-2006}). 

In the thermodynamic limit, due to dephasing, it is natural to expect that the 
equilibrium value of any local observable ${\mathcal O}$ is described by the 
so-called diagonal ensemble as 
%
\begin{equation}
\label{d-ensemble}
...
hamiltonian ${\mathcal H}$. For generic, i.e., non-integrable, models 
...
quasi-local integrals of motion strongly affects the dynamics, preventing the onset of 
thermal behavior. It has been suggested in Ref.~\cite{rigol-2007,rigol-2008} that   
in this situation the post-quench steady-state can be described by the so-called 
Generalized Gibbs Ensemble ($GGE$) as 
%
\begin{equation}
\label{gge}
...
still not settled. A recent work~\cite{ilievski-2015a} suggests that the $GGE$ 
description of the steady state  is complete provided that the so-called quasi-local 
charges~\cite{prosen-2014,pereira-2014,ilievski-2015} are included in~\eref{gge}. 

Valuable insights into this issue have been provided by the so-called quench-action 
approach~\cite{caux-2013}. For Bethe ansatz integrable models this allows to construct 
the diagonal ensemble average~\eref{d-ensemble} in the thermodynamic limit, provided 
that the overlaps $\langle\lambda|\Psi_0\rangle$ are known. The physical idea is that 
for large system sizes it is possible to approximate the sum over the eigenstates 
in~\eref{d-ensemble} using a saddle point argument. Specifically, one starts with 
rewriting~\eref{d-ensemble} as 
%
\begin{equation}
\label{de1}
...
denoting the real part. For typical initial states $|\Psi_0\rangle$
one has ${\mathcal E}\propto L$, with $L$ the system size, reflecting  a typical 
vanishing of the overlaps as $\langle\Psi_0|\lambda\rangle\propto e^{-c L}$. 
Similar to the standard Thermodynamic Bethe Ansatz (TBA)~\cite{taka-book}, the 
extensivity of 
${\mathcal E}$ suggests that in the thermodynamic limit~\eref{de1} is dominated 
by a saddle point. Surprisingly, for some Bethe ansatz solvable models and some 
simple initial states standard TBA techniques, 
complemented with analytical knowledge of the overlaps $\langle\Psi_0|\lambda
\rangle$, allow to determine {\it analytically} this saddle point, and for some  
local observables, the diagonal ensemble expectation values~\eref{de1}, in the 
thermodynamic limit. This has been done succesfully for instance in the Lieb-Liniger 
model for the quench from the Bose-Einstein condensate~\cite{de-nardis-2014} 
(BEC) and for the $XXZ$ spin chain for the quench from the N\'eel state~\cite{wouters-2014,
brockmann-2014}. Interestingly, recently it has been suggested that the quench action 
approach could allow to reconstruct the full relaxation dynamics to the steady 
state~\cite{de-nardis-2015a,de-nardis-2015}. 

%Formally,~\eref{de1} is equivalent to the standard Gibbs 
%ensemble after replacing ${\mathcal H}\to{\mathcal E}$. A crucial difference is 
%that while typical hamiltonians are local, ensuring the extensivity of the energy, 
%this is not guaranteed for ${\mathcal E}$. 


Here we provide a Monte Carlo implementation of the quench action approach in the 
finite-size spin-$1/2$ Heisenberg chain ($XXX$ chain). combining Monte Carlo methods and Bethe ansatz techniques. 
...
and the Majumdar-Ghosh (MG) $|MG\rangle$ state (cf.~\eref{mg}). In particular, we specialize 
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection{The low-density regime ({\bf PROBLEM !!!})}
\label{sec:low-density}

It is interesting to consider the situation with $M/L\to\infty$ (low-density limit). 
We first consider the case with all the particles forming a single zero-momentum 
string. Due to parity invariance only strings of even length are allowed. 
We assume the string hypothesis~\eref{str-hyp}, restricting ourselves to zero-momentum 
strings, i.e., 
%
\begin{equation}
\lambda^{j}_{n}=-i(n-1-2j)+i\delta_{n}^j,
\end{equation}
%
with $n$ the length of the string and $j$ labelling the different string components. 
Due to parity invariance we restrict ourselves to half strings, i.e., $j=1,\dots,n/2$.
We verified by explicit computation that for a single $m$ string one has (at leading order 
in the string deviations) 
%
\begin{equation}
\det(G^+)=-\frac{2^{n/2}}{\delta_n^{n/2}}L\prod_{j>k=1}^{n/2-1}(\delta_n^k-\delta_n^j)^{-1},
\end{equation}
%
and 
%
\begin{equation}
\det(G^-)=-\frac{2^{n/2}}{\delta_n^{n/2}}(L-1)\prod_{j>k=1}^{n/2-1}(\delta_n^k-\delta_n^j)^{-1}.
\end{equation}
%
This implies that 
%
\begin{equation}
\frac{\det_n(G^+)}{\det_n(G^-)}\to\frac{L}{L-1},\quad\forall n.
\end{equation}
%
However, it is straightforward to check that  
%
\begin{equation}
\prod_{j=1}^{n/2}\frac{\sqrt{\lambda_j^2+1}}{4\lambda_j}\propto\sqrt{\delta_n^{n/2}}.
\end{equation}
%
{\bf The prefactor does not seem to be a simple function of $n$}.


We conclude that Bethe states corresponding to a single string have zero N\'eel 
overlap. 

A problem arises if one considers the Majumdar-Ghosh state. The overlap is obtained 
rescaling the N\'eel overlap (see below) with the prefactor
%
\begin{equation}
\prod\limits_{j=1}^n\frac{1}{2}
\Big(1-\frac{\lambda_j-i}{\lambda_j+i}\Big). 
\end{equation}
%
Note that the product above is over all the string components. 
For a single string one gets 
%
\begin{equation}
\prod\limits_{j=1}^n\frac{1}{2}
\Big(1-\frac{\lambda_j-i}{\lambda_j+i}\Big)\propto\frac{1}{\delta_n^{n/2}}.
\end{equation}
%
Putting everything together one would get a diverging MG overlap which is clearly wrong. 
