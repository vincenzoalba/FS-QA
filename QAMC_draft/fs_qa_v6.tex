...
%{\mathcal O}\rangle_{DE}=\langle\Psi_0|{\mathcal O}|\Psi_0\rangle$. 
...
charges~\cite{prosen-2014,pereira-2014,ilievski-2015,essler-2015} are included in the GGE~\eref{gge}. 
...
For concreteness reasons, we focus on the isotropic Heisenberg point ($XXX$), i.e. Hamiltonian \eref{xxx-ham} with $\Delta=1$, 
...
%The key idea is to sample the eigenstates with no zero-momentum 
...
%\equiv-\ln\langle\alpha|\Psi_0\rangle$, with $\Re$ denoting the real part. 
...
%Note also that the N\'eel state, $Q_n^{(0)}$ can be 
...
of the Bethe equations (see section~\ref{sec:1.2}). It is straightforward to check that the
...
length is $M/2$ because of parity invariance. Note also that in determining $q$, strings 
...
\bibitem{ga-15}
G. Goldstein and N. Andrei, % Equilibration and generalized Gibbs ensemble for hard wall boundary conditions,
...
\bibitem{de_luca} A. De Luca, G. Martelloni, and J. Viti, %Stationary states in a free fermionic chain from the quench action method.
...
\bibitem{dmcf-06}
G. De Chiara, S. Montangero, P. Calabrese, and R. Fazio,  %Entanglement Entropy dynamics in Heisenberg chains, 
